...
import { getFirestore, collection, getDocs, query, where, limit, orderBy, documentId } from 'firebase/firestore/lite';
...
export class FirestoreService {
  ...
  //get last song heard home page
  async getLastSongHeardHome() {
    const songCol = collection(this.db,'song');
    const q = query(songCol, orderBy('dateEcoute', 'desc'), limit(3));
    const songSnapshot = await getDocs(q);
    const songList = songSnapshot.docs.map((doc) => doc.data());
    console.log("Voici le getLastSongHeardHome : ", songList);
    return songList;
  };
  ...
  async getLastSongHeard() {
    const songCol = collection(this.db,'song');
    const q = query(songCol, orderBy('dateEcoute', 'desc'));
    const songSnapshot = await getDocs(q);
    const songList = songSnapshot.docs.map((doc) => doc.data());
